...
public OnPluginStart()
{
	...
	g_hCvarEnabled = CreateConVar("sm_enhancedsprays_enabled", "1", "Enable Enhanced Sprays\n0 = Disabled\n1 = Enabled", _, true, 0.0, true, 1.0);
	g_bEnabled = GetConVarBool(g_hCvarEnabled);
	HookConVarChange(g_hCvarEnabled, OnConVarChange);

	g_hCvarDistance = CreateConVar("sm_enhancedsprays_distance", "150", "Distance players can apply sprays to walls", _, true, 0.0);
	g_fDistance = GetConVarFloat(g_hCvarDistance);
	HookConVarChange(g_hCvarDistance, OnConVarChange);

	g_hCvarDelay = CreateConVar("sm_decalfrequency", "0", "How often players can apply sprays", _, true, 0.0);
